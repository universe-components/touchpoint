...
TPP协议秉承 “ 一切皆行为 ” 的理念，实现了真正意义上的自适应智能体协作模型。

- **状态 - 角色驱动模型：** 通过状态 - 角色驱动模型，多智能体之间完全可以根据不同场景、环境自主调整优化它们之间的协作关系。
- **兼容互联网时代主流技术栈：** TPP协议主要用Java实现，完全兼容互联网时期的主流编程语言，及操作系统。基于注解，开发者可以通过最小的App改动，将App转变为智能体。
- **颠覆传统功能扩展方式：** TPP协议颠覆性地提出了协作者范式，通过该范式，开发者可以在线平滑、无损地扩展智能体功能。
...
- **支持多种Agent文件格式：** 支持apk、jar、exe、ipa、py等文件格式。
