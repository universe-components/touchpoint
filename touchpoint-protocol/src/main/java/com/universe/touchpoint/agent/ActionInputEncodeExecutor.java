...
public class ActionInputEncodeExecutor {
  ...
  public static Double[] encode(
      ...
      LangModelConfig langModelConfig) {
    Double[][] fused_visual_features =
        VisionInputEncodeExecutor.encode(imageData, visionModelConfig, visionLangModelConfig);
    ...
    try (Jep jep = new SharedInterpreter()) {
      ...
      return (Double[])
          jep.getValue(
              "np.concatenate((fused_visual_features, text_embedding.reshape(1, -1)), axis=1)");
