...
public class ActionInstructionDecoder implements AIModelOutputDecoder<ActionSequence, Double[]> {
  ...
  public ActionSequence run(String params, Class<Double[]> clazz) {
    Double[] actionData =
        Arrays.stream(params.split(", ")).map(Double::parseDouble).toArray(Double[]::new);
    ...
    // 创建位置变化对象
    ActionSequence.Position deltaPosition =
        new ActionSequence.Position(actionData[0], actionData[1], actionData[2]);

    // 创建角度变化对象
    ActionSequence.Orientation deltaOrientation =
        new ActionSequence.Orientation(actionData[3], actionData[4], actionData[5]);
