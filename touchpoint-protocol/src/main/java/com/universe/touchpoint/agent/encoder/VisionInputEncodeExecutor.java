  ...
  public static Double[][] encode(
    ...
    try (Jep jep = new SharedInterpreter()) {
      for (Map.Entry<String, VisionInputEncoder<?>> encoder : encoders.entrySet()) {
        List<Class<?>> types =
            ClassUtils.getInterfaceGenericTypes(
                Objects.requireNonNull(encoders.get(encoder.getKey())).getClass());
        if (types.contains(VisionModelConfig.class)) {
          jep.set(
              encoder.getKey(),
              ((VisionInputEncoder<VisionModelConfig>) encoder.getValue())
                  .run(imageData, visionModelConfig));
        } else {
          jep.set(
              encoder.getKey(),
              ((VisionInputEncoder<VisionLangModelConfig>) encoder.getValue())
                  .run(imageData, visionLangModelConfig));
        }
      }
      // 加载 Python 脚本
