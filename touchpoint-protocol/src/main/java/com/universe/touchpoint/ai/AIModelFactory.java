...
public class AIModelFactory {
  ...
  private static final Map<AIModelType, Class<? extends AIModel<?, ?, ?, ?>>> modelMap =
      new HashMap<>();
  ...
  public static <C, REQ, CH> AIModel<?, REQ, C, CH> getModel(LangModelConfig modelConfig) {
    synchronized (lock) {
      if (model == null) {
        try {
          // 获取模型类类型
          Class<? extends AIModel<?, REQ, C, CH>> modelClass =
              (Class<? extends AIModel<?, REQ, C, CH>>) modelMap.get(modelConfig.getType());
          if (modelClass == null) {
          ...
          throw new IllegalArgumentException(
              "Error creating model for type: " + modelConfig.getType(), e);
        }
      }
