...
public class OpenVLAChoiceParser implements ChoiceParser<OpenVLA.ActionResponse> {
  ...
  public Pair<List<AgentAction<?, ?>>, AgentFinish<?>> parse(
      OpenVLA.ActionResponse choices, AgentAction<?, ?> currentAction) {
    List<AgentAction<?, ?>> agentActions = new ArrayList<>();

    AgentAction<OpenVLA.ActionResponse, ?> correctAction =
        (AgentAction<OpenVLA.ActionResponse, ?>) currentAction;
