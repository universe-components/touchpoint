...
public class PromptBuilder {
  ...
  private static final Map<AIModelType, Class<? extends PromptGenerator<?>>> modelGeneratorMap =
      new HashMap<>();
    ...
    synchronized (lock) {
      if (promptGenerator == null) {
        try {
          ...
          // 获取模型类类型
          Class<? extends PromptGenerator<?>> modelGeneratorClass =
              modelGeneratorMap.get(modelType);
          ...
          throw new IllegalArgumentException(
              "Error creating prompt generator for type: " + modelType, e);
        }
      }
