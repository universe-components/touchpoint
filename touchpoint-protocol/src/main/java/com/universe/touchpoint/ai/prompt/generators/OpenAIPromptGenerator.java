...
public class OpenAIPromptGenerator implements PromptGenerator<String> {
  ...
  @Override
  public <I, O> String generatePrompt(
      @Nonnull List<AgentActionMeta> nextActions, AgentAction<I, O> action, String question) {
    StringBuilder toolList = new StringBuilder();
    StringBuilder agentNames = new StringBuilder();

    for (AgentActionMeta actionMetaInfo : nextActions) {
      toolList
          .append(actionMetaInfo.getName())
          .append(": ")
          .append(actionMetaInfo.getDesc())
          .append("\n");
    ...
    String finalFormatInstructions =
        OpenAITemplate.FORMAT_INSTRUCTIONS.replace("{agent_names}", agentNames.toString());
