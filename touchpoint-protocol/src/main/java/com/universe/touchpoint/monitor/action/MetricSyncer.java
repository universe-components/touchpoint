...
public class MetricSyncer extends AgentActionExecutor<AgentAction<?, ?>, TouchPoint> {
    ...
    public TouchPoint run(AgentAction<?, ?> action, TouchPointContext context) {
        ...
        SocketProtocol protocol = Objects.requireNonNull(ConfigManager.selectAgentSocket(task)).getBindProtocol();
        ...
        ((AgentSyncProtocol<Pair<TaskMetric, Map<String, ActionMetric>>>) AgentSyncProtocolSelector.selectProtocol(protocol)).send(Pair.of(taskMetric, actionMetrics), task);
        return action;
    }
