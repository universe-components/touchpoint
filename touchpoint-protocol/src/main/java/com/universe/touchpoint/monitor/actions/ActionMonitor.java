...
public class ActionMonitor<T extends TouchPoint> implements ActionChecker<T, MonitorResult> {
    ...
    public MonitorResult run(T touchPoint, TouchPointContext context) {
        AgentActionMetaInfo actionMetaInfo = context.getActionMeta(touchPoint.getState().getCtxName());
        ActionMetricConfig metricConfig = ConfigManager.selectActionMetricConfig(actionMetaInfo.getActionName(), touchPoint.getTask());
        MonitorResult monitorResult = new MonitorResult();

        assert metricConfig != null;
        if (context.getActionMetric(touchPoint.getState().getCtxName()).getPredictionCount() > metricConfig.getMaxPredictionCount()) {
