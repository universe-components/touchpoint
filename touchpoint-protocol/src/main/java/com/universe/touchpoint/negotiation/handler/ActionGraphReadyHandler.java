...
public class ActionGraphReadyHandler<Config>
    implements AgentSocketStateHandler<AgentActionMeta, Map<String, Config>> {

  @Override
  public <C extends AgentContext> Map<String, Config> onStateChange(
      AgentActionMeta actionMeta, C agentContext, String task) {
    if (actionMeta != null) {
              ...
              toAction -> {
                if (metaRegion.containsTouchPointSwapAction(toAction)) {
                  ActionGraphBuilder.getTaskGraph(task)
                      .addEdge(actionMeta, metaRegion.getTouchPointSwapAction(toAction));
                }
              });
