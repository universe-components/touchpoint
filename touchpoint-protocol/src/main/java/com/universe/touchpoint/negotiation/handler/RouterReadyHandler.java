...
public class RouterReadyHandler implements AgentSocketStateHandler<ActionGraph, Boolean> {
  ...
  public <C extends AgentContext> Boolean onStateChange(
      ActionGraph actionGraph, C actionContext, String task) {
    ...
    if (actionGraph != null) {
      ...
      AgentActionMeta actionMetaInfo =
          metaRegion.getTouchPointAction(taskActionContext.getAction());
      ...
      TransportConfig<?> transportConfig =
          ConfigManager.selectTransport(
              taskActionContext.getAction(), taskActionContext.getBelongTask());
      TouchPointTransportRegistry<?> registry =
          TouchPointTransportRegistryFactory.getRegistry(transportConfig.transportType());

      predecessors.forEach(
          action -> registry.register(actionMetaInfo, action.getName(), task, true));
      successors.forEach(
          action -> registry.register(actionMetaInfo, action.getName(), task, false));
      for (Map.Entry<AgentActionMeta, List<AgentActionMeta>> entry :
          actionGraph.getAdjList().entrySet()) {
