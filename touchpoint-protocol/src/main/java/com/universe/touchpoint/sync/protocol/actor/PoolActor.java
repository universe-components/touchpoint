...
public class PoolActor<M> extends AbstractBehavior<M> {
  ...
  public <C extends AgentContext> PoolActor(
      ActorContext<M> context, C agentContext, String filter, String topic, Class<M> messageType) {
    super(context);
    var router =
        Routers.pool(
            5, new ActorReceiver<>(context, agentContext, filter, topic, messageType).create());
    this.messageType = messageType;
    receiverPool = context.spawn(router, "receiver-pool");
  }

  public static <C extends AgentContext, M> Behavior<M> create(
      C agentContext, String filter, String topic, Class<M> messageType) {
    return Behaviors.setup(
        context -> new PoolActor<>(context, agentContext, filter, topic, messageType));
  }
