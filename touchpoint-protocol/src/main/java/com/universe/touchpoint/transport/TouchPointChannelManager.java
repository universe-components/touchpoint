...
public class TouchPointChannelManager {
  ...
  public static void registerContextReceiver(
      String actionName, String actionClassName, String task) {
    try {
      Class<?> tpInstanceReceiverClass = Class.forName(actionClassName);
      RoleExecutor<?, ?> tpInstanceReceiver =
          (RoleExecutor<?, ?>) tpInstanceReceiverClass.getConstructor().newInstance();
