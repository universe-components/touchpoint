  ...
  public void registerBeanDefinitions(
    ...
    if (importingClassMetadata.hasAnnotation(TouchPointAgent.class.getName())) {
      ...
      Map<String, Object> agentAttributes =
          importingClassMetadata.getAnnotationAttributes(agentClassName);
      assert agentAttributes != null;
      String agentName = (String) agentAttributes.get("name");
      AgentAnnotationMeta agentAnnotationMeta =
          new AgentAnnotationMeta(agentClass, agentAttributes);
      try {
        ...
        ((MetaRegion) TouchPointMemory.getRegion(Region.META))
            .putTouchPointAgent(agentName, agentMeta);
      } catch (Exception ex) {
